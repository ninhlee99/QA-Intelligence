...
/**
 * ADR-017 §6, applied to the Repository seam: every backing adapter SHALL
 * pass the same lifecycle, idempotency, optimistic-concurrency, and
 * Workspace identity contract suite. This is that shared suite; each
 * adapter's test file supplies only a factory.
 */
export function runEvaluationCampaignRepositoryContract(
  adapterName: string,
  makeRepository: () => EvaluationCampaignRepository | Promise<EvaluationCampaignRepository>,
): void {
  test(`[${adapterName}] creates and loads one immutable Workspace-bound campaign record`, async () => {
  ...
  test(`[${adapterName}] transitions canonically with optimistic revision and idempotent retry`, async () => {
    const repository = await makeRepository();
  ...
  test(`[${adapterName}] a delayed create retry returns the original retained operation result`, async () => {
    const repository = await makeRepository();
    const request = createRequest();
    const created = await repository.create(request);
    assert.ok(created.ok);
  ...
  test(`[${adapterName}] records an attributable trial boundary without changing campaign state`, async () => {
    const repository = await makeRepository();
  ...
  test(`[${adapterName}] recovers only from retained safe trial boundaries`, async () => {
    const repository = await makeRepository();
    await advanceToRunning(repository);
    const command = {
      workspace_id: "workspace-evaluation-001",
  ...
  test(`[${adapterName}] blocks recovery instead of rerunning an active trial with unknown effects`, async () => {
    const repository = await makeRepository();
    await advanceToRunning(repository);
  ...
  test(`[${adapterName}] the same campaign identity remains isolated across Workspaces`, async () => {
    const repository = await makeRepository();
  ...
  test(`[${adapterName}] cannot become ready while any governed version remains unresolved`, async () => {
    const repository = await makeRepository();
    const request = createRequest();
  ...
  test(`[${adapterName}] rejects campaign and trial state changes without attributable evidence`, async () => {
    const transitionRepository = await makeRepository();
    ...
    const boundaryRepository = await makeRepository();
  ...
  test(`[${adapterName}] binds command idempotency input and never reopens a terminal campaign`, async () => {
    const repository = await makeRepository();
...
async function advanceToRunning(repository: EvaluationCampaignRepository): Promise<void> {
  const created = await repository.create(createRequest());
  assert.ok(created.ok);
  let revision = 1;
